...
void ICACHE_RAM_ATTR brzo_i2c_write(const uint8_t *data, uint32_t no_of_bytes, bool repeated_start)
{
	...
	// Assembler Variables
	uint32_t a_set = a_set, a_repeated = a_repeated, a_in_value = a_in_value, a_temp1 = a_temp1, a_bit_index = a_bit_index;
...
void ICACHE_RAM_ATTR brzo_i2c_read(uint8_t *data, uint32_t nr_of_bytes, bool repeated_start)
{
	...
	if (i2c_error > 0) return;
	// Assembler Variables
	uint32_t a_set = a_set, a_repeated = a_repeated, a_in_value = a_in_value, a_temp1 = a_temp1, a_temp2 = a_temp2, a_bit_index = a_bit_index;
	a_temp2 = 0;
	if (repeated_start == true) a_repeated = 1;
	else a_repeated = 0;
...
void ICACHE_RAM_ATTR brzo_i2c_ACK_polling(uint16_t ACK_polling_time_out_usec) {
	...
	uint32_t a_set = a_set, a_in_value = a_in_value, a_temp1 = a_temp1, a_bit_index = a_bit_index;
...
#endif
{
	...
	// Assembler Variables
	uint32_t a_set = a_set, a_temp1 = a_temp1;
