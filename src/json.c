...
static int init_string(json_stream *json)
{
    ...
    if (json->data.string == NULL) {
        json->data.string_size = INITIAL_STRING_SIZE;
...
long json_get_long(json_stream *json)
{
    char *p = json->data.string;
...
static int user_get(struct json_source *source)
{
    source->position++;
    int c = source->source.user.get(source->source.user.ptr);
    return c;
}

static int user_peek(struct json_source *source)
{
    int c = source->source.user.peek(source->source.user.ptr);
    return c;
}
